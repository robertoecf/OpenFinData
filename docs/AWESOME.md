...
| [OBM](https://obm.com.br) | BR cross-asset portal | 4 | 3 | no | Public site + `llms.txt` (2026-09-04). `/api` not documented on the public surface; `robots.txt` Disallow `/api/`. Day-0 snapshot: [`snapshots/obm_day0_robots_sitemap.json`](snapshots/obm_day0_robots_sitemap.json). No in-tree survey note on this branch. |
...
1. Public pages, Terms of Use, privacy, `robots.txt`, `llms.txt`, FAQ,
   pricing, and advertised machine surfaces only. Terms override robots
   when they forbid crawlers or systematic extract.
2. Write a survey note and, if the UI teaches something, a product-ref
   note. Targeted human-readable GETs of legal/docs/landing paths. Do not
   scrape the data corpus.
3. Add one row here. Link only files that exist on the same branch. Do not
   implement an adapter in the same change unless official public data is
   missing and the vendor surface is licensed for that use.
